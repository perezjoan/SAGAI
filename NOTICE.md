...
4. Builds the `task_specs` list by calling `uvlm.prompts.build_prompt()` for each task, combining the shared role instruction with the per-task prompt, theory, and format strings.
5. Sets generation parameters (max tokens, temperature, top-p, seed).
